...
def callback(in_data, frame_count, time_info, status):
    ...
    data_l = np.concatenate((npdata_in[:, 2][:, np.newaxis],
                             npdata_in[:, 3][:, np.newaxis],
                             npdata_in[:, 4][:, np.newaxis]),
                            axis=1)
    data_r = np.concatenate((npdata_in[:, 5][:, np.newaxis],
                             npdata_in[:, 6][:, np.newaxis],
                             npdata_in[:, 7][:, np.newaxis]),
                            axis=1)
    ...
    # npdata_out[:, 0] = npdata_in[:, 5]
    # npdata_out[:, 1] = npdata_in[:, 6]
...
dma_L = rt.BeamformerDMA2(samp_freq=RATE,
                          nb_buffsamp=samp_per_buffer,
                          mic_dist=0.013,
                          nullangle_v=np.array([153., 106.]))
dma_R = rt.BeamformerDMA2(samp_freq=RATE,
                          nb_buffsamp=samp_per_buffer,
                          mic_dist=0.013,
                          nullangle_v=np.array([153., 106.]))
