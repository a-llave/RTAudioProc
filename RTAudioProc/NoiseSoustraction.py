...
class NoiseSoustraction:
    def __init__(self, nb_fft, nb_channels, samp_freq, threshold_f=6, gr_timecste_f=0.5, bypass=False, verbose=False):
        # GENERAL
        self.bypass = bypass
        ...
        self.noisedb_prev = np.zeros((self.nb_freq, nb_channels))
        self.db_atk_f = 3.
        self.db_rls_f = 10.
        # SMOOTH GR IN FREQ
        N = self.nb_fft / 10.
        ...
        self.gr_coef_f = 1 / (1 + self.samp_freq * self.gr_timecste_f / self.nb_fft)
        self.gain_redu_prev = np.zeros((self.nb_freq, self.nb_channels))
        # FOR PLOT
        self.magdb_inp = np.zeros((self.nb_freq, self.nb_channels))
        self.magdb_out = np.zeros((self.nb_freq, self.nb_channels))
        # SPECIAL
    ...
    def processfft(self, fft_inp):
        ...
        if not self.bypass:
            ...
            # ---- SMOOTH GR IN FREQ
            toto = np.zeros((self.nb_freq + len(self.win_v) - 1, self.nb_channels))
            ...
            gain_redu = toto[idx:idx + self.nb_freq, :]
            # ---- SMOOTH GR IN TIME
